(() => {
  if (document.body?.dataset?.page !== "energy-home") return;
  ...
  function applyEnglishLanding() {
    ...
    for (const node of nodes) {
      ...
      if (RO_TO_EN[trimmed]) node.nodeValue = raw.replace(trimmed, RO_TO_EN[trimmed]);
    }
    document.querySelectorAll("[placeholder]").forEach((element) => {
      const value = element.getAttribute("placeholder") || "";
      if (RO_TO_EN[value]) element.setAttribute("placeholder", RO_TO_EN[value]);
    });
  }

  window.addEventListener("lacurent:languagechange", (event) => {
    if (event.detail?.language === "en") applyEnglishLanding();
  });
