...
def test_dense_parametric_heating_grid_contains_exactly_1000_non_sku_nodes() -> None:
    nodes = build_parametric_heating_nodes()

    assert len(nodes) == HEATING_PARAMETRIC_NODE_TOTAL == 1000
    assert {item["technology_id"] for item in nodes} == {
        "condensing-gas",
        "heat-pump-air-water",
        "heat-pump-air-air",
        "electric-boiler",
...
def test_heating_technologies_consume_dense_grid_without_turning_nodes_into_products() -> None:
