...
FORM={
  "project_name":"Electric refine probe","locality_id":"siruta-54984","locality":"Cluj-Napoca",
...
def post_json(path, payload, timeout=60):
  ...
  req=urllib.request.Request(BASE+path,data=raw,method="POST",
    headers={"Content-Type":"application/json","User-Agent":"lacurent-electric-refine-probe"})
  t=time.perf_counter()
  try:
    with urllib.request.urlopen(req,timeout=timeout) as r:
      body=r.read().decode()
      return r.status, json.loads(body), round(time.perf_counter()-t,3), len(raw)
  except urllib.error.HTTPError as e:
    body=e.read().decode(errors="replace")
    try: parsed=json.loads(body)
    except Exception: parsed={"raw":body[:2000]}
    return e.code, parsed, round(time.perf_counter()-t,3), len(raw)

for run in range(1,9):
  prior=[]
  for phase in ("axis","halton"):
    st,b,sec,n=post_json("/api/optimization/home-lab/branch",{
      "form":FORM,"branchId":"electric-boiler","searchPhase":phase,"priorCandidates":[]
    })
    print(json.dumps({"run":run,"phase":phase,"status":st,"seconds":sec,"request_bytes":n,
      "eval":b.get("parametricEvaluations"),"candidates":len(b.get("candidates") or [])},ensure_ascii=False))
    if st!=200:
      print(json.dumps({"body":b},ensure_ascii=False)); break
    prior.extend(b.get("candidates") or [])
  else:
    st,b,sec,n=post_json("/api/optimization/home-lab/branch",{
      "form":FORM,"branchId":"electric-boiler","searchPhase":"refine","priorCandidates":prior
    })
    print(json.dumps({"run":run,"phase":"refine","status":st,"seconds":sec,"request_bytes":n,
      "prior_count":len(prior),"eval":b.get("parametricEvaluations"),
      "candidates":len(b.get("candidates") or []),"body_if_error":None if st==200 else b},ensure_ascii=False))
