...
import type {
  ...
  EmissionClassResult,
  EnergyClassificationComparison,
  EnergyClassificationResult,
  EnergyPerformanceClassificationInput,
  GlobalEnergyClassResult,
  ReferenceBuildingClassificationResult,
  ServiceEnergyClassInput,
  ServiceEnergyClassResult
} from "../model/EnergyClassification";
import type {
...
const SERVICE_DEFAULT_STATUS: Record<EnergyServiceForClassification, ClassificationStatus> = {
...
function trace<T>(input: {
  ...
  source: string;
  sourceType: "registry_default" | "calculated" | "internal_estimate";
}): CalculationTrace<T> {
  return input;
}
...
function wrapGlobalResult(result: ReturnType<typeof classifyEstimatedEnergyClass>): GlobalEnergyClassResult {
  return {
...
function classifyEmissionClass(
  co2KgM2Year: number | null | undefined,
  thresholdSets: EmissionClassThresholdSet[] = EMISSION_CLASS_THRESHOLD_SETS
): EmissionClassResult {
  const assumptions = [
  ...
  if (thresholdSets.length === 0) {
    ...
    return {
      ...
      assumptions,
      confidence: "low",
      trace: trace<EstimatedEmissionClass | "unknown">({
        value: "unknown",
        unit: "class",
...
function serviceInputFor(
  input: EnergyPerformanceClassificationInput,
  service: EnergyServiceForClassification
): ServiceEnergyClassInput | undefined {
...
function classifyServiceEnergyClass(
  service: EnergyServiceForClassification,
  input: ServiceEnergyClassInput | undefined,
  thresholdSets: ServiceEnergyClassThresholdSet[] = SERVICE_ENERGY_CLASS_THRESHOLD_SETS
): ServiceEnergyClassResult {
  const assumptions = [
  ...
  if (!input || input.isApplicable === false) {
    ...
    return {
      ...
      warnings,
      confidence: "low",
      trace: trace<EstimatedEnergyClass | "unknown">({
        value: "unknown",
        unit: "class",
...
function classifyReferenceBuilding(input: EnergyPerformanceClassificationInput): ReferenceBuildingClassificationResult {
...
function distanceToNextBetterClass(
  primaryEnergyKwhM2Year: number | null | undefined,
  thresholdSet: EnergyClassThresholdSet | undefined,
...
function compareRealToReference(
  input: EnergyPerformanceClassificationInput,
...
export function classifyEnergyPerformance(input: EnergyPerformanceClassificationInput): EnergyClassificationResult {
  ...
  const services = SERVICES.reduce((map, service) => {
    ...
    return map;
  }, {} as EnergyClassificationResult["services"]);
