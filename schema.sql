...
CREATE TABLE building_platform_reprocessing_jobs (
...
CREATE INDEX building_platform_reprocessing_jobs_owner_idx
ON building_platform_reprocessing_jobs(owner_user_id, created_at);

CREATE INDEX building_platform_reprocessing_jobs_project_idx
ON building_platform_reprocessing_jobs(project_id, created_at);

CREATE INDEX building_platform_reprocessing_jobs_status_idx
ON building_platform_reprocessing_jobs(status, created_at);

CREATE TABLE building_platform_export_manifests (
...
CREATE INDEX building_platform_export_manifests_created_idx
ON building_platform_export_manifests(created_at);

CREATE INDEX building_platform_export_manifests_checksum_idx
ON building_platform_export_manifests(manifest_checksum);

CREATE TABLE heating_products (
    id TEXT PRIMARY KEY,
    external_id TEXT,
...
CREATE INDEX heating_products_active_technology_power_idx
ON heating_products(active, technology_id, rated_power_kw);

CREATE INDEX heating_products_external_id_idx
ON heating_products(external_id);

CREATE TABLE heat_pump_performance_points (
...
CREATE INDEX heat_pump_performance_product_idx
ON heat_pump_performance_points(product_id, outdoor_temperature_c, flow_temperature_c);

CREATE TABLE heat_pump_seasonal_performance (
    product_id TEXT NOT NULL,
...
CREATE INDEX heat_pump_seasonal_product_idx
