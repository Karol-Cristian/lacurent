...
| Reviewed executable fixtures | 20 |
...
Twenty narrowed, reviewed executable fixtures now exist:
...
`INVESTIGATION_009_LEVEL_1_EXPLICIT_INPUT_PACK` defines the required pack before Level 1 helper composition. `FIXTURE_016_LEVEL_1_CORE_COMPONENT_ORCHESTRATOR` implements the narrow core centered on transmission, ventilation, and final/primary/CO2 with blockers preserved. `FIXTURE_017_LEVEL_1_MONTHLY_HEATING_ORCHESTRATION` extends that pack only with optional monthly heating summary composition over Fixture 006 rows. `FIXTURE_018_LEVEL_1_FAIL_CLOSED_HARDENING` hardens that boundary against incomplete, ambiguous, inconsistent, or invented input packs. `FIXTURE_020_REGISTRY_CONTRACT_INPUT_BUILDER_GATE` starts the Phase C pre-orchestration gate by validating registry record shape and rejecting derived values as normal raw auditor input. `FIXTURE_021_ENVELOPE_FROM_AUDITOR_INPUT` starts Phase D by preparing only source-backed exterior envelope transmission inputs after the Phase C gate. Class assignment, DHW useful/display-only rows, RER display reconciliation, and utility threshold recalculation remain outside the core orchestrator unless every helper input is source-backed. Complete envelope/Htr readiness, full DHW final energy, lighting, cooling systems, reference building, Level 2 full-auditor behavior, certificate/CPE workflow, report generation, UI, Worker, DB/schema, API, deploy, and production integration remain blocked.
